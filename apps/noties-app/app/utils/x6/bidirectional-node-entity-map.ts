...
export class BidirectionalNodeEntityMap<Entity extends { id: Id }> {
  ...
  set(nodeType: PersonNodeType, node: X6Node, entity: Entity) {
    this.map.set(node, entity, nodeType);
  }

  getNode(nodeType: PersonNodeType, entityId: Entity["id"]) {
    return this.map.getBy2(entityId, nodeType);
  }

  getEntity(nodeType: PersonNodeType, nodeId: X6Node["id"]) {
